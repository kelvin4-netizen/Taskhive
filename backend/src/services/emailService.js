...
// ── Use Resend HTTP API (more reliable than SMTP on cloud hosts) ──────────────
const sendViaResend = async ({ to, subject, html }) => {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.EMAIL_FROM || 'TaskHive <onboarding@resend.dev>',
      to: [to],
      subject,
      html,
    }),
...
<head>
  ...
  <style>
    ...
    .logo { font-size: 22px; font-weight: 900; color: #D4A843; margin-bottom: 28px; }
    .logo span { color: #fff; }
...
const send = async ({ to, subject, html }) => {
  try {
    if (!process.env.SMTP_PASS) {
      logger.warn(`Email skipped (no SMTP_PASS configured) — would send "${subject}" to ${to}`);
      return;
    }
...
const sendVerificationEmail = async (email, name, token) => {
  ...
  await send({
    ...
    html: baseTemplate(`
      ...
      <hr class="divider"/>
      <p style="font-size:12px;color:#555;">Or copy this link into your browser:<br/><a href="${url}" style="color:#D4A843;word-break:break-all;">${url}</a></p>
